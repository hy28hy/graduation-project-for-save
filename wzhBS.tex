...
经过 $T$ 个时间步的累积，最终的 $z_T$ 将退化为纯各向同性的高斯噪声。而我们要训练的核心，就是去噪网络，它需要在这个低维空间中，根据我们提供的空间与语义条件，预测并剥离这些噪声。
...
该 U-Net 的主干（Backbone）采用了标准的下采样-上采样对称结构，并配备了跳跃连接（Skip Connections）。为了注入条件特征，我们对网络结构进行了针对性改造，主要体现在空间特征的零卷积注入与语义特征的交叉注意力引导两个方面。

针对空间结构特征的注入，采用了类似 ControlNet 的零卷积（Zero-Convolution）残差注入策略。空间特征图 $F_{spatial}$ 具有与原始图像相同的拓扑布局，且其内部已经通过掩码机制滤除了局部的异常纹理，仅保留了全局解剖先验。具体而言，对于 U-Net 中的每一个编码器块（Encoder Block），我们将该层的中间特征与经过下采样的 $F_{spatial}$ 进行通道维度上的拼接或相加。为了避免在训练初期，未经过充分训练的条件分支对原本稳定的去噪主干造成灾难性干扰，特征在注入前会经过一个权重和偏置均初始化为 0 的 $1 \times 1$ 卷积层（即 $\mathcal{Z}_{conv}$）：
...
对于语义文本特征的引导，情况则有所不同。由 VLM 生成的文本描述特征 $C_{sem}$ 是离散的序列向量，缺乏空间拓扑信息。为了将其融入二维的生成过程中，我们在 U-Net 的各个分辨率层级中嵌入了交叉注意力机制（Cross-Attention Mechanism）。在此机制中，U-Net 的中间层特征展平后作为查询（Query），而 $C_{sem}$ 作为键（Key）和值（Value）。这一机制在语义层面上发挥着“纠偏”作用：当模型在某个局部区域试图重构出类似肿瘤的高亮特征时，注意力机制会强制该区域的特征去匹配文本描述中关于“健康、均匀、低信号”的键值对，从而将其压制并转换为正常组织的纹理。
...
\subsection{扩散模型训练参数设置与训练结果（待补充）}
\label{subsec:model_training}
在前述章节完成网络架构设计后，本节将详细介绍多条件引导的潜在扩散模型在 BraTS 2021 脑部 MRI 数据集上的具体训练设置与收敛情况。模型的训练过程旨在使其在由空间特征与文本语义构成的双重条件约束下，稳定地学习到正常脑部解剖结构的潜在流形分布。

首先，在硬件与软件环境方面，本实验所有模型的训练与验证均在搭载多张图形处理器的计算节点上完成（具体使用了 4 张独立显卡并行计算），深度学习框架采用 PyTorch Lightning，并配置了分布式数据并行（Distributed Data Parallel, DDP）策略以加速训练过程。为了保证实验结果的严格可重复性，全局随机种子统一设定为 1，并且将底层线程数（如 OMP、OPENBLAS 等）严格限制为 1 以避免多进程资源抢占冲突。

其次，在核心超参数设置上，为了保证去噪网络在复杂的双重条件引导下平稳收敛，模型采用了全量参数微调（Full Fine-tuning）策略，未引入任何局部参数冻结。优化过程的初始学习率精确设定为 $1 \times 10^{-5}$，每个 GPU 设备的训练批次大小（Batch Size）设置为 12，对应的数据加载器工作线程数（Num Workers）设为 4。此外，模型共设定了 500 个全局训练轮次（Max Epochs），在训练过程中采用单步梯度累积（Accumulate Grad Batches = 1），并全程使用 32 位浮点精度计算以保障医学图像特征的数值稳定性。

特别值得注意的是，为了防止模型对特定的语义条件产生过拟合，增强后续基于无分类器引导（CFG）推理的鲁棒性，我们在训练过程中引入了随机条件丢弃机制。具体而言，实验设定了 0.2 的模板替换概率（Template Probability），这意味着在训练迭代中，有 20\% 的概率将由 Qwen3-VL-30B 模型生成的细粒度稠密医学描述（Caption）替换为简单的基础模板。这一机制强制模型在依赖局部空间特征与依赖全局通用先验之间进行动态权衡，从而全面提升模型的泛化能力。

在模型评估与保存策略方面，系统每间隔 25 个训练轮次自动执行一次全面的测试集评估。监控指标重点关注验证准确率（val\_acc），并利用模型检查点机制（Model Checkpoint）自动提取并保存训练期间表现最优的一组权重文件（save\_top\_k = 1）。整个训练过程的标量数据通过 TensorBoard 和 CSV 格式同步持久化记录，以便于后续的曲线分析与状态恢复。

图 \ref{fig:training_loss} 展示了模型在训练过程中的损失函数下降与验证指标变化曲线。

\begin{figure}[htbp]
    \centering
    \fbox{\parbox[c][5.5cm]{0.8\textwidth}{\centering 待补充图片：训练过程的 Loss 下降曲线与验证集准确率（val\_acc）曲线图。展示横轴为 Epoch，纵轴为数值，呈现收敛趋势。}}
    \caption{多条件引导潜在扩散模型的训练收敛曲线}
    \label{fig:training_loss}
\end{figure}

从训练记录可以看出，在训练的初始阶段，由于引入了随机初始化的空间引导分支与交叉注意力语义分支，模型损失存在一定的波动；但随着训练的推进（约在第 \textbf{[请补充：例如 150]} 个 Epoch 后），损失函数呈现出显著且稳定的下降趋势，最终在设定的 500 个 Epoch 结束前充分收敛于较低的误差水平。此时，模型在验证集上达到了 \textbf{[请补充：例如 95.5\%]} 的最优准确率，表明多条件融合网络已经成功克服了模态间的异质性，具备了在潜在空间中精准重构健康解剖结构的能力，为后续的无监督异常推理与像素级病灶定位奠定了坚实的模型基础。

\subsection{基于残差的无监督异常推理与精确定位机制}
\label{subsec:anomaly_scoring}

在完成模型的联合训练后，双重条件引导的潜在扩散模型已经充分掌握了健康脑部 MRI 的数据流形。本节将详细阐述在无监督推理阶段，如何利用该模型对包含未知肿瘤的测试图像进行受控重构，并基于重构残差实现高精度的像素级异常定位。整个推理与定位机制在逻辑上可划分为截断加噪重构、多维度异常评分计算以及形态学后处理三个连续的步骤。

首先进行的是基于截断时间步的受控加噪与重构。在测试阶段，给定一张待测的脑部切片 $x_{test}$，我们的目标是生成一张对应的健康版本 $\hat{x}_{test}$。为了在“保留患者特异性宏观解剖结构”与“彻底抹除异常病灶特征”之间取得最佳平衡，本研究采用了截断部分加噪（Truncated Partial Noising）策略。通过 VAE 编码器获取测试图像的潜在表示 $z_0 = \mathcal{E}(x_{test})$ 后，我们不执行完整的 $T$ 步正向加噪，而是设定一个截断时间步 $\tau$（其中 $0 < \tau < T$）。根据前向扩散的闭式解，我们直接向 $z_0$ 添加对应强度的高斯噪声 $\epsilon \sim \mathcal{N}(0, I)$，得到带噪潜在变量 $z_\tau$：
$$z_\tau = \sqrt{\bar{\alpha}_\tau} z_0 + \sqrt{1 - \bar{\alpha}_\tau} \epsilon$$
截断时间步 $\tau$ 是一个至关重要的超参数。若 $\tau$ 过小，病灶的高频纹理未被完全破坏，模型在去噪时会直接发生恒等映射，导致肿瘤被原样重构从而引发漏检；若 $\tau$ 过大，原本健康的脑沟回等个性化细节将完全丢失，导致重构图像与原图产生巨大的背景偏差从而引发误报。通过引入合理的 $\tau$，我们将肿瘤区域隐匿于噪声之中，为后续的修复创造条件。随后，以 $z_\tau$ 为起点，注入由测试图像提取的空间特征 $F_{spatial}$ 以及由全局健康提示词生成的语义特征 $C_{sem}$，启动双重条件引导的逆向去噪过程。经过 $\tau$ 步的逐步降噪，模型输出完全符合健康解剖分布的潜在表示 $\hat{z}_0$。最后，通过 VAE 解码器将其映射回像素空间，得到重构图像 $\hat{x}_{test} = \mathcal{D}(\hat{z}_0)$。

重构完成后，为了精准定位肿瘤，需要构建联合空间与结构的多维度异常评分函数来量化原始图像 $x_{test}$ 与重构图像 $\hat{x}_{test}$ 之间的差异。传统的异常检测往往仅使用像素级的绝对误差（L1 距离），但在医学影像中，由于扩散模型在生成过程中不可避免地会产生轻微的纹理偏移或亮度波动，单一的像素比较极易在脑室边缘或皮层折叠处产生大量假阳性（False Positives）噪点。为此，本研究构建了一个结合绝对误差与结构相似性（Structural Similarity Index Measure, SSIM）的联合异常评分函数。对于图像中的任意像素坐标 $(i, j)$，多维度异常评分 $\mathcal{S}(i, j)$ 的计算公式定义为：
$$\mathcal{S}(i, j) = \lambda \cdot \mathcal{R}_{L1}(i, j) + (1 - \lambda) \cdot \mathcal{R}_{SSIM}(i, j)$$
其中，$\lambda$ 为平衡系数（实验中经验性取值为 0.6）。绝对残差项 $\mathcal{R}_{L1}(i, j) = \left| x_{test}(i, j) - \hat{x}_{test}(i, j) \right|$ 衡量了局部的信号强度突变，对胶质瘤等高信号病灶极其敏感；而 $\mathcal{R}_{SSIM}(i, j)$ 是基于局部窗口计算的结构差异度，公式如下：
$$\mathcal{R}_{SSIM}(i, j) = 1 - \frac{(2\mu_x\mu_{\hat{x}} + c_1)(2\sigma_{x\hat{x}} + c_2)}{(\mu_x^2 + \mu_{\hat{x}}^2 + c_1)(\sigma_x^2 + \sigma_{\hat{x}}^2 + c_2)}$$
SSIM 考虑了相邻像素间的亮度（$\mu$）、对比度（$\sigma$）和结构相关性（$\sigma_{x\hat{x}}$），能够有效抑制由于轻微空间未对齐而引发的背景噪声响应，从而显著提升了评分图（Score Map）的总体信噪比。

最后，执行形态学后处理以生成最终的肿瘤掩膜。经过上述计算得到的初始评分图 $\mathcal{S}$ 通常存在孤立的噪点和边缘伪影，为了获得符合临床诊断标准的连续肿瘤边界，需要对其进行一系列空间滤波与形态学操作。首先，对评分图应用核大小为 $5 \times 5$ 的中值滤波器（Median Filter）以平滑局部尖峰噪声；其次，采用自适应阈值分割算法（Otsu's Method）自动寻找最佳阈值 $th$，将连续的浮点型评分图转化为二值化的初始掩膜 $M_{init}$：
$$M_{init}(i, j) = \begin{cases} 1, & \text{if } \mathcal{S}_{filtered}(i, j) > th \\ 0, & \text{otherwise} \end{cases}$$
接着，对 $M_{init}$ 依次执行形态学开运算（先腐蚀后膨胀），以彻底消除面积小于特定阈值的微小假阳性区域；再执行形态学闭运算（先膨胀后腐蚀），以填补肿瘤内部可能出现的空洞现象。经过上述后处理步骤得到的二值图即为算法输出的最终脑肿瘤定位掩膜（Predicted Mask），至此完成了从图像输入到病灶定位的端到端无监督检测闭环。

\begin{figure}[htbp]
    \centering
    \fbox{\parbox[c][5.5cm]{0.8\textwidth}{\centering 待补充图片：异常定位流程与效果展示。依次并列展示：(a) 包含肿瘤的原始图像 $x_{test}$；(b) 重构出的健康图像 $\hat{x}_{test}$；(c) 多维度异常评分图 $\mathcal{S}$；(d) 经过形态学后处理的最终肿瘤定位掩膜。}}
    \caption{基于多条件引导的无监督异常定位推理流程示例。联合 L1 距离与 SSIM 结构相似性的多维度评分机制能够有效抑制正常组织的轻微偏移，精准指示肿瘤轮廓。}
    \label{fig:anomaly_localization_result}
\end{figure}

\subsection{本章小结}

本章在完成空间特征与语义特征提取模块设计的基础上，详细阐述了多条件引导下的潜在扩散重构模型的整体构建、训练策略与推理机制，是实现本课题无监督脑肿瘤检测的核心枢纽。

本章引入了基于预训练 VAE 的感知压缩技术。将高分辨率的 T2-FLAIR 脑部切片映射至低维潜在空间进行扩散运算，这一操作不仅大幅滤除了医学图像中的高频干扰噪声，更从根本上解决了像素级扩散模型计算开销大、推理速度慢的性能瓶颈。

针对跨模态条件的融合难题，本章重新设计了 U-Net 去噪网络架构。提出利用零卷积模块残差注入空间结构特征，为模型提供刚性的脑部拓扑轮廓约束；同时利用多头交叉注意力机制注入文本语义特征，为模型提供柔性的健康组织纹理约束。在训练策略上，本章详细设定了模型在 BraTS 2021 数据集上的分布式全量微调参数，并创新性地引入了 0.2 概率的随机条件丢弃机制，强制模型在局部空间线索与全局通用先验之间进行动态权衡，确保了损失函数的平稳收敛。双重条件的协同工作与鲁棒的训练策略有效阻断了肿瘤特征的异常传递，防止了重构过程中的结构漂移现象。

最后，本章设计了基于残差的无监督异常推理与精确定位机制。通过截断部分加噪策略破坏肿瘤高频特征，并在双重约束下进行受控的逆向去噪。为了进一步提升定位精度，构建了结合绝对像素误差（L1）与结构相似性度量（SSIM）的联合异常评分函数，并辅以自适应阈值分割与形态学操作。该套完整的推理机制能够有效滤除由于图像微小未对齐产生的背景假阳性，最终实现了对脑肿瘤的高精度像素级定位。
% ------------------------------
...
脑部肿瘤的早期精准诊断对提升患者生存率至关重要。针对传统有监督深度学习方法在脑部 MRI 异常检测中存在的高昂标注成本、长尾罕见病灶泛化能力差以及闭集识别限制等临床痛点，本文深入探索了无监督异常检测范式。结合当前生成式人工智能领域最前沿的去噪扩散概率模型（DDPM）与视觉-语言模型（VLM），本文提出并构建了一种基于多条件引导潜在扩散模型（LDM）的脑部 MRI 肿瘤无监督异常检测与定位算法，实现了“以正推异”的精准病灶捕获。

回顾全文，本文的主要研究工作与创新结论体现在以下三个方面：

首先，针对传统重构模型在处理医学图像时容易产生“恒等映射”从而导致病灶漏检的问题，本文提出并设计了基于领域掩码网络的空间特征条件建模方法。通过构建基于切比雪夫距离的注意力掩码矩阵，物理阻断了局部高频异常纹理的传递路径；并结合多尺度卷积网络与 Transformer 全局聚合机制，迫使模型利用脑部的长程依赖与对称性进行推断。该机制成功提取了纯粹的健康解剖全局先验，有效解决了重构过程中的结构漂移现象。

其次，针对纯视觉模型在医学解剖认知上的“语义盲目性”，本文创新性地引入了 Qwen3-VL 视觉-语言大模型，构建了细粒度医学语义引导模块。通过设计包含医学先验的专家提示工程，将复杂的脑部影像特征转化为高维文本语义向量，并利用多头交叉注意力机制将其注入 U-Net 去噪网络。同时，配合随机条件丢弃与无分类器引导（CFG）策略，成功将庞大的医疗文本先验知识跨模态对齐到视觉生成过程中，赋予了模型强大的异常纠偏能力。

最后，本文在低维潜在空间中搭建了融合双重条件的扩散重构框架，建立了一套鲁棒的无监督异常推理闭环。通过截断加噪策略破坏肿瘤局部纹理，并在空间轮廓与健康语义的双重约束下进行逆向去噪修复。结合绝对像素误差（L1）与结构相似性（SSIM）的联合异常评分函数，以及自适应形态学后处理，算法能够有效滤除空间未对齐产生的背景噪声。在 BraTS 2021 数据集上的实验验证表明，本算法在仅使用健康数据训练的前提下，显著提升了脑肿瘤在图像级别与像素级别的检测与定位精度，展现了极高的临床辅助诊断潜力。

\subsection{未来展望}

尽管本文提出的基于多条件引导的潜在扩散模型在脑部 MRI 无监督异常检测中取得了阶段性的优异成果，但受限于计算资源、研究时间以及现有生成模型的固有特性，本研究仍存在一些局限性，需要在未来的工作中进行更深入的探索与完善。

一方面，本文的研究主要集中在二维（2D）切片层面的图像重构与检测。然而，临床获取的磁共振影像在物理空间上是连续的三维（3D）体素数据。将 2D 切片独立处理会不可避免地丢失层间的空间连贯性（Inter-slice Context），导致部分跨层面的微小病灶在三维重组时出现边缘不连续。未来的研究可以考虑构建原生的 3D 潜在扩散模型，或者引入时间/深度维度的注意力机制（如 Video-Diffusion 架构），以直接在三维空间中拟合健康脑部的体积流形分布，从而进一步提升三维病灶定位的准确性与平滑度。

另一方面，本文主要利用了对肿瘤水肿区域高度敏感的 T2-FLAIR 单一成像模态。在实际的神经肿瘤学诊断中，医生通常需要综合分析 T1 加权、T1 增强（T1ce）以及 T2 加权等多模态序列，以实现对肿瘤核心、坏死区及强化边缘的精确分级。未来可以探索多模态特征融合机制，将不同序列的互补信息共同作为扩散模型的引导条件，从而将当前的“单分类异常定位”拓展为“细粒度病灶成分无监督分析”。

此外，扩散模型由于依赖多步马尔可夫迭代去噪，其推理速度相较于前馈神经网络（如 U-Net）仍然较慢。虽然本文引入了潜在空间压缩与截断加噪策略，但在面对海量临床筛查任务时，计算开销依然是一个挑战。未来可以尝试引入一致性模型（Consistency Models）、流匹配（Flow Matching）或基于常微分方程（ODE）的高效采样算法，在不损失重构保真度的前提下，将采样步数压缩至个位数，以满足临床实时辅助诊断的需求。

最后，生成式 AI 的临床落地需要极高的鲁棒性。未来的工作应当收集包含不同磁共振设备厂商（如西门子、GE、飞利浦）、不同场强（1.5T 与 3.0T）的多中心医学数据集，对模型进行更加严苛的交叉验证。同时，进一步优化视觉-语言模型（VLM）的提示工程，探索全自动的异常分析报告生成机制，使人工智能不仅能“圈出病灶”，更能为放射科医生提供具有可解释性的诊断依据。

\begin{acknowledgment}

敲下致谢这部分内容时，心里有一种终于长舒一口气的轻松，但更多的是不舍。随着这篇论文的定稿，我的本科四年也即将画上句号。

在此，我想向我的指导老师谢琳副教授表达最由衷的感谢。谢老师始终给予我极大的耐心与包容，每次汇报阶段性进展时，她总能凭借敏锐的学术直觉一针见血地指出我逻辑上的漏洞，并在我遇到瓶颈、面对不理想的重构结果感到气馁时给予我鼓励。谢老师严谨治学的态度与高远的学术视野，是我科研道路上最宝贵的一课。

在具体的代码落地与实验推进中，我要特别感谢伍晨旭师兄。从为我梳理庞杂的服务器环境、协调宝贵的算力资源，到无数个下午帮我一行行排查那些令人崩溃的代码 Bug，晨旭师兄倾注了大量的时间与精力。在这篇论文的背后，是他毫无保留的经验分享和一次次不厌其烦的指导，才让那些原本停留在纸面上的公式最终跑通成了真实的实验数据。师兄踏实、纯粹的科研态度，也让我深刻体会到了实验室里最真挚的科研传承。

我还要深深感谢我的母校西安交通大学，以及生命科学与技术学院。在交大度过的四年时光，不仅赋予了我扎实的生物医学工程专业基础，更将“精勤求学、敦笃励志”的校训刻进了我的底色。这里的梧桐道、实验室和图书馆，见证了我从一个大一新生到如今独立完成毕业设计的蜕变。

最后，感谢四年来陪伴我的同学、朋友，以及永远在背后默默支持我的家人。关山初度尘未洗，策马扬鞭再奋蹄。愿所有给予过我帮助的人未来一切顺利，愿我们都能在各自的旷野中热烈生长。
...
\addcontentsline{toc}{section}{\thesection \quad 外文原文}

\includepdf[
    pages=-, 
    scale=0.99,      % 关键：缩小比例，给页眉页脚留出空间
    pagecommand={\thispagestyle{main}\sectionmark{附录A 外文原文}},
    offset=0 0cm   % 可选：根据需要上下移动 PDF 内容的位置
]{original_paper_clean.pdf}

% 插入外文翻译（保留页眉页脚）
\clearpage
\phantomsection
\refstepcounter{section}
\addcontentsline{toc}{section}{\thesection \quad 外文翻译}

\includepdf[
    pages=-, 
    scale=0.99, 
    pagecommand={\thispagestyle{main}\sectionmark{附录B 外文翻译}}
]{translated_paper.pdf}

% --- 附录C 程序代码 ---
\clearpage
\phantomsection
\refstepcounter{section} % 计数器增加到 C
\addcontentsline{toc}{section}{\thesection \quad 程序代码}
\sectionmark{程序代码} % 更新页眉

% 设置当前页面的页眉页脚样式
\thispagestyle{main}

\begin{center}
    {\sthree \bfseries 附录C \quad 程序代码} % 在页面上方打印一个标题
\end{center}

\vspace{1em}


\lstinputlisting{train_MRI_foundation_caption.py}
\lstinputlisting{generate_MRI_captions_qwen30B.py}
\lstinputlisting{MRIad_dataloader_caption.py}
\clearpage
% --- 附录部分结束 ---
